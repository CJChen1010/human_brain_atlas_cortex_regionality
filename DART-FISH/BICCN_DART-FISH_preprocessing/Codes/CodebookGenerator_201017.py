...
def create_jsoncodebook(infilepath,outfilepath, totalCycles = 1, offCycles = 0, 
    ...
    if not uniColorAllowed:
        if offCycles > 0:
            ...
            barcodelist = barcodelist.loc[barcodelist.nunique(axis = 1) >= 1]
...
barcode_file = '_codebook/HB_Feb2022.txt'
codebook_file = '_codebook/HB_Feb2022.json'
create_jsoncodebook(infilepath=barcode_file, outfilepath=codebook_file,
                    totalCycles=8, offCycles=4, firstCycleAnchor=False,
                    addEmptyBarcodes=True, uniColorAllowed=False)
